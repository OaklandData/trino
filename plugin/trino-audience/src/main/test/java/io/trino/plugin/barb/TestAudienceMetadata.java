...
public class TestAudienceMetadata
{
    private static final AudienceTableHandle NUMBERS_TABLE_HANDLE = new AudienceTableHandle("example", "numbers");
    private AudienceMetadata metadata;

    @BeforeMethod
    public void setUp()
            throws Exception
    {
        URL metadataUrl = Resources.getResource(TestAudienceClient.class, "/example-data/example-metadata.json");
        assertNotNull(metadataUrl, "metadataUrl is null");
        AudienceClient client = new AudienceClient(new AudienceConfig().setMetadata(metadataUrl.toURI()), CATALOG_CODEC);
        metadata = new AudienceMetadata(client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new AudienceColumnHandle("text", createUnboundedVarcharType(), 0),
                "value", new AudienceColumnHandle("value", BIGINT, 1)));

        // unknown table
        assertThatThrownBy(() -> metadata.getColumnHandles(SESSION, new AudienceTableHandle("unknown", "unknown")))
                .isInstanceOf(TableNotFoundException.class)
                .hasMessage("Table 'unknown.unknown' not found");
        assertThatThrownBy(() -> metadata.getColumnHandles(SESSION, new AudienceTableHandle("example", "unknown")))
                .isInstanceOf(TableNotFoundException.class)
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new AudienceTableHandle("unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new AudienceTableHandle("example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new AudienceTableHandle("unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new AudienceColumnHandle("text", createUnboundedVarcharType(), 0)),
                new ColumnMetadata("text", createUnboundedVarcharType()));
